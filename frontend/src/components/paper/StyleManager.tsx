...
export function StyleManager({ styles, onChanged }: {
  ...
  return (
    <div className="space-y-3">
      <div className="text-sm font-semibold text-ink">Styles</div>

      {error && (
        <div className="rounded-lg border border-danger/30 bg-danger/5 px-2 py-1.5 text-[11px] text-danger">{error}</div>
      )}

      <div>
        <div className="mb-1 text-[10px] font-semibold uppercase tracking-wide text-faint">Built-in</div>
        <div className="flex flex-wrap gap-1.5">
          {builtins.map((s) => (
            <span key={s.id} className="flex items-center gap-1.5 rounded-lg border border-line bg-surface px-2 py-1 text-[11px] text-ink">
              {s.name}
              <button onClick={() => openEditor(s.id)} title="Duplicate & customise"
                      className="text-faint hover:text-ink">✎</button>
            </span>
          ))}
        </div>
      </div>
      ...
      {custom.length > 0 && (
        <div>
          <div className="mb-1 text-[10px] font-semibold uppercase tracking-wide text-faint">My styles</div>
          <div className="space-y-1.5">
            {custom.map((s) => (
              <div key={s.id} className="rounded-lg border border-line bg-surface-2 p-2">
                <div className="flex items-center gap-1.5">
                  ...
                  <button onClick={() => openEditor(s.id)} className="text-faint hover:text-ink">✎</button>
                  <button onClick={() => remove(s.id)} disabled={busy}
                          className="text-faint hover:text-danger">×</button>
                </div>
                {s.derived_from && (
                  <div className="mt-1 text-[9px] text-muted">
                    Learned from <span className="font-medium text-ink">{s.derived_from}</span>
                    {s.detected && (
                      <>
                        {' — read '}
                        {s.detected.split(',').filter(Boolean).map((d) => (
                          <span key={d} className="mr-1 rounded border border-line bg-surface px-1 text-ink">
                            {d.replace(/_/g, ' ')}
                          </span>
                        ))}
                        <span className="text-faint">· the rest came from the base style</span>
                      </>
                    )}
                  </div>
                )}
      ...
      <div className="rounded-lg border border-dashed border-line-strong p-3">
        <div className="mb-2 text-[11px] font-semibold text-ink">Define your own format</div>
        <div className="flex flex-wrap items-center gap-2">
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Style name"
                 className="min-w-32 flex-1 rounded-lg border border-line bg-surface px-2 py-1.5 text-[11px] text-ink outline-none placeholder:text-faint focus:border-line-strong" />
          <select value={base} onChange={(e) => setBase(e.target.value)}
                  className="rounded-lg border border-line bg-surface px-2 py-1.5 text-[11px] text-ink outline-none focus:border-line-strong">
            {builtins.map((s) => (
              <option key={s.id} value={s.id} className="bg-surface text-ink">base: {s.name}</option>
            ))}
          </select>
          <input ref={fileRef} type="file" accept=".docx" hidden
                 onChange={(e) => upload(e.target.files?.[0])} />
          <button onClick={() => fileRef.current?.click()} disabled={busy}
                  className="rounded-lg bg-accent px-3 py-1.5 text-[11px] font-medium text-accent-fg hover:opacity-90 disabled:opacity-45">
      ...
      {editing && (
        <div className="space-y-2 rounded-lg border border-line bg-surface-2 p-3">
          <div className="text-[11px] font-semibold text-ink">Edit stylesheet JSON</div>
          <textarea value={draft} onChange={(e) => setDraft(e.target.value)} rows={12}
                    className={clsx('w-full rounded-lg border border-line bg-surface p-2',
                                    'font-mono text-[10px] leading-relaxed text-ink outline-none focus:border-line-strong')} />
          <div className="flex gap-2">
            ...
            <button onClick={() => { setEditing(null); setDraft('') }}
                    className="rounded-lg border border-line bg-surface px-3 py-1.5 text-[11px] text-muted hover:bg-surface-2">
              Cancel
            </button>
          </div>
        </div>
