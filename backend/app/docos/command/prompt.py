...
SYSTEM = (
    ...
    "- 'Increase the headings by 2' is a STEP, not a size: resize with "
    "params.delta=2 (negative to reduce). params.font_size is only for a size "
    "the request states outright — reading 'by 2' as font_size=2 sets the "
    "document in 2 pt type. 'Larger' with no number is params.scale.\n"
    ...
    "- A request about a table's rules — 'only the top and bottom borders', "
    "'remove the gridlines', 'no vertical lines' — is `border` on target 'table', "
    "with params.sides naming the edges to draw (top, bottom, left, right, "
    "inside_h, inside_v) and params.width in points; 0 draws none. Naming some "
    "sides means those and no others.\n"
    "  params.sides also takes 'header' — the rule under the heading row, "
    "which a paper draws and Word has no table-level name for — and may be "
    ...
    "two heavy rules and no bold type anywhere.\n"
    "- Changing letter case — 'capitalise the headings', 'make these uppercase', "
    "'sentence case the captions' — is `case` with params.kind in "
    "upper|lower|title|sentence. Do NOT use rewrite: the words do not change.\n"
    "- Line spacing and the gaps around paragraphs are `spacing`: params.line "
    "(1, 1.5, 2), params.before_pt, params.after_pt. 'Double space the document' "
    "is spacing with params.line=2, not a rewrite.\n"
    "- A typeface — 'set the body in Times New Roman' — is format with "
    "style.font_family.\n"
